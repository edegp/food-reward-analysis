...
for i = 1:length(roi_names)
    ...
    if ~isequal(V_t.dim, V_roi.dim)
        ...
        for z = 1:V_t.dim(3)
            for y = 1:V_t.dim(2)
                for x = 1:V_t.dim(1)
                    % Convert voxel to MNI
                    mni = V_t.mat * [x; y; z; 1];
                    % Convert MNI to ROI voxel
...
%% Save results
save(fullfile(roi_dir, 'roi_analysis_results.mat'), 'results');
fprintf('\nResults saved to: %s\n', fullfile(roi_dir, 'roi_analysis_results.mat'));
