function matpath = make_glm_nutri_run(subs, run_label, model_dir, model_name, cnt, model_name_tmp)
%% Create and save names/onsets/durations/pmod for a single subject-run
%% Model: Value + protein + fat + carbs + kcal
...
% model_name: name of the glm folder (e.g. 'glm_nutri')
% cnt: run count (1,2,...)
% model_name_tmp: optional filename base for saved .mat (e.g. 'glm_nutri_1')
    ...
    candidate_dirs = { fullfile('../../Food_Behavior', ['sub-', subs]) , ...
                       fullfile('../../Food_Behavior', subs) };
    beh_dir = '';
    ...
    if isempty(beh_dir)
        error('make_glm_nutri_run:missingBehaviorDir', ...
    ...
    if isempty(files_Rate_list)
        error('make_glm_nutri_run:missingRating', 'No rating_data*.csv found for subject %s', subs);
    end
    ...
    if isempty(files_Time_list)
        error('make_glm_nutri_run:missingTiming', 'No GLM_all*.csv found for subject %s', subs);
    end
    ...
    fprintf('make_glm_nutri_run: subject %s cnt=%d\n  time: %s\n  rate: %s\n', subs, cnt, time_filepath, rate_filepath);
    ...
    % pmod: Value + protein + fat + carbs + kcal
    pmod(1).name{1} = 'Value';
    pmod(1).param{1} = data_tmp.RatingValue(idx_nomiss);
    pmod(1).poly{1} = 1;

    pmod(1).name{2} = 'Protein';
    pmod(1).param{2} = data_tmp.image_protein(idx_nomiss);
    pmod(1).poly{2} = 1;

    pmod(1).name{3} = 'Fat';
    pmod(1).param{3} = data_tmp.image_fat(idx_nomiss);
    pmod(1).poly{3} = 1;

    pmod(1).name{4} = 'Carbs';
    pmod(1).param{4} = data_tmp.image_carbs(idx_nomiss);
    pmod(1).poly{4} = 1;

    pmod(1).name{5} = 'Kcal';
    pmod(1).param{5} = data_tmp.image_kcal(idx_nomiss);
    pmod(1).poly{5} = 1;
