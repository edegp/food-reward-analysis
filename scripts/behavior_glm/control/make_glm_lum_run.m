function matpath = make_glm_lum_run(subs, run_label, model_dir, model_name, cnt, model_name_tmp)
%% Create and save names/onsets/durations/pmod for a single subject-run
%% Model: Value + Luminance
...
% model_name: name of the glm folder (e.g. 'glm_lum')
% cnt: run count (1,2,...)
% model_name_tmp: optional filename base for saved .mat (e.g. 'glm_lum_1')
    ...
    if isempty(beh_dir)
        error('make_glm_lum_run:missingBehaviorDir', ...
    ...
    if isempty(files_Rate_list)
        error('make_glm_lum_run:missingRating', 'No rating_data*.csv found for subject %s', subs);
    end
    ...
    if isempty(files_Time_list)
        error('make_glm_lum_run:missingTiming', 'No GLM_all*.csv found for subject %s', subs);
    end
    ...
    fprintf('make_glm_lum_run: subject %s cnt=%d\n  time: %s\n  rate: %s\n', subs, cnt, time_filepath, rate_filepath);
