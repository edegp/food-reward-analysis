function matpath = make_glm_rgb_run(subs, run_label, model_dir, model_name, cnt, model_name_tmp)
...
% model_name: name of the glm folder (e.g. 'glm_rgb')
% cnt: run count (1,2,...)
% model_name_tmp: optional filename base for saved .mat (e.g. 'glm_rgb_1')
    ...
    if isempty(beh_dir)
        error('make_glm_rgb_run:missingBehaviorDir', ...
    ...
    if isempty(files_Rate_list)
        error('make_glm_rgb_run:missingRating', 'No rating_data*.csv found for subject %s', subs);
    end
    ...
    if isempty(files_Time_list)
        error('make_glm_rgb_run:missingTiming', 'No GLM_all*.csv found for subject %s', subs);
    end
    ...
    fprintf('make_glm_rgb_run: subject %s cnt=%d\n  time: %s\n  rate: %s\n', subs, cnt, time_filepath, rate_filepath);
