function dir_name = run_second_level(subs_list, model_name, con_title)
    ...
    for c = 1:length(con_title)
        ...
        % Make Directory for the RFX (robust path construction)
        ts = char(datetime('now','Format','yyyyMMdd_HHmmss'));
        % Ensure components are char vectors (not cells/strings) and scalar
        ...
        % Validate dir_name to avoid mkdir errors (must be non-empty 1xN char or string scalar)
        if ~( (ischar(dir_name) && isrow(dir_name) && ~isempty(dir_name)) || (isstring(dir_name) && isscalar(dir_name) && strlength(dir_name) > 0) )
            error('run_second_level:InvalidDirName', 'Constructed dir_name is invalid for mkdir: %s', mat2str(dir_name));
        end
        ...
        for i = 1:length(subs_list)
            ...
            % SPM.matの読み込み (search only within the specified model)
            ...
            con_name = '';  % Initialize before loop
            for cc = 1:num_cons
            ...
            if isempty(con_name)
                error('run_second_level:ContrastNotFound', ...
        ...
        cfg_util('savejob', job_id, fullfile(dir_name, 'secondlevel_batch.m'));    % 再現用 matlab スクリプトを生成
