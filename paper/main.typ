...
#import "./header.typ": appendix, make_cover, side_by_side
...
#set_body(
  block()[
    ...
    *期待される貢献*
    ...
    == DNNモデルの訓練
    === 交差検証とデータ拡張
    データリークを防ぐため、まず896枚の元画像を8分割し、各フォールドで7分割を訓練用、1分割を検証用とした。データ拡張は訓練フォールドのみに適用し、検証フォールドには元画像のみを使用した。これにより、同一元画像由来の拡張版が訓練・検証間で重複することを防いだ。
    ...
    === モデル訓練
    事前学習済みのConvNeXt-Base、ResNet152、およびVGG16モデルを使用し、最終の全結合層を主観的価値スコア（1–8）を出力するように置き換え、全層をファインチューニングした。損失関数にはHuber損失（δ = 1）を使用し、AdamWオプティマイザを用いて学習を行った。学習率は1e-4、ミニバッチサイズは373（GPUメモリ制約による）、エポック数は250とした。8分割交差検証における各検証フォールドからの予測を連結し、実際の評価との相関を計算した。計算環境は、Intel Xeon w5-2465XプロセッサとRTX 4000 Ada GPUを搭載し、Python 3.12.7およびPyTorch 2.5.0（CUDA対応）を使用した。
    ...
    === ファインチューニングモデルの評価
    DCNNモデル（ConvNeXt、ResNet、VGG）の性能は、8分割交差検証における予測評価と実際の評価との相関係数（Pearsonの相関係数）で評価した。各モデルの予測精度を比較し、最も高い相関を示したモデルを特定した。
    ...
    == DNNモデルによる主観的価値予測
    3つのDCNNモデル（ConvNeXt、ResNet、VGG）とCLIPを用いて、食品画像から主観的価値を予測した（@fig:acc）。事前学習済みモデルの比較では、CLIPモデルが最も高い予測精度を示し、予測と実際の評価との相関係数は*r = 0.77 ± 0.02*であった。一方、事前学習のみのDCNNモデルはそれぞれConvNeXt *r = 0.63 ± 0.07*、ResNet *r = 0.44 ± 0.07*、VGG *r = 0.18 ± 0.06*と低い精度を示した。食品評価データでファインチューニングを行った結果、DCNNモデルの予測精度は大幅に向上し、ConvNeXt *r = 0.69 ± 0.04*、ResNet *r = 0.67 ± 0.06*、VGG *r = 0.56 ± 0.04*となった。これらの結果は、CLIPが事前学習のみで高い予測精度を達成する一方、DCNNはファインチューニングにより性能が向上することを示している。

    #side_by_side(
      image("./image/cv_barplot_pretrained_vs_finetuned.png", width: 70%),
      <fig:acc>,
      image("./image/encoding_lineplot_convnext_clip_sidebyside.png", width: 110%),
      <fig:repr1>,
      "a. DNNモデルによる主観的価値予測の精度（予測と実際の評価との相関係数）。青：事前学習のみ、緑：ファインチューニング後。CLIPは事前学習済みモデルのみ使用。 b. DNNモデルの各層における情報表現。左：ファインチューニング後のConvNeXt、右：事前学習済みCLIP。各線は、各属性の予測と実際の評価との相関係数を示す。",
      columns: 1,
      gutter: 2%,
    )
    ...
    #side_by_side(
      ...
      <fig:glm_value>,
      image("./image/roi_effectsize_rgb_nutri_ImagexValue.png", width: 90%),
      <fig:roi_value>,
      "a. 主観的価値（Image × Value）に関連する脳活動。クラスターレベルFWE補正（p < 0.05、クラスター形成閾値p < 0.001 uncorrected）。カラーバーはT値を示す。色付きの輪郭線は関心領域（ROI）を示す：vmPFC（青）、OFC（緑）、左線条体（シアン）、右線条体（マゼンタ）。b. ROI解析における主観的価値の説明力。すべてのROIで有意な効果が認められた（p < 0.05）。vmPFC：腹内側前頭前皮質、mOFC：内側眼窩前頭皮質。",
      columns: 1,
      gutter: -1%,
    )
    ...
    ===== ROI-RSA結果
    二重中心化を適用したRSA解析の結果、視覚野において最も高い説明率が観察された。V1では3モデルとも約65–69%の説明率を達成し、CLIPが67.0%、ImageNetが64.3%、Foodが68.7%であった。初期視覚野でも同様に、CLIPが62.5%、ImageNetが63.5%、Foodが68.7%の説明率を示した。
    ...
    #side_by_side(
      ...
      <fig:rsa_roi_centered>,
      "ROI-RSA解析結果（二重中心化適用）。a. 各ROIにおける3モデル（CLIP：視覚言語モデル、ImageNet：ImageNet事前学習済みConvNeXt、Food：食品画像でファインチューニングしたConvNeXt）の説明率の比較。b. 左は視覚野のみにおける平均説明率、右は全31 ROIにおける平均説明率。エラーバーは標準誤差（SEM）。*** p < 0.001、* p < 0.05（対応のあるt検定）。",
      columns: 1,
      gutter: 0%,
    )
    ...
    ===== ConvNeXtモデル
    ConvNeXtモデルの階層的解析結果を@fig:hierarchical に示す。各層グループに関連する共有成分を加えた解析の結果、初期層は後頭葉の一次視覚野（V1）および外側後頭皮質で有意な効果を示した。中間層は側頭葉および頭頂葉で活性化を示した。後期層は広範な活性化を示し、視覚野から側頭葉、頭頂葉にわたる領域で有意な効果が認められた。最終層は最も広範な活性化を示し、腹内側前頭前皮質（vmPFC）を含む前頭葉領域でも有意な効果が認められた。

    これらの結果は、ConvNeXtの階層構造が脳の視覚処理階層と対応しており、初期層が低次視覚野、後期層が高次視覚野および価値関連領域と相関することを示唆している。

    ===== CLIPモデル
    CLIPモデルの階層的解析結果を@fig:hierarchical に示す。初期層は後頭葉で最も強い活性化を示した。中間層は側頭葉および頭頂葉で広範な活性化を示し、視覚的オブジェクト認識に関連する領域との対応が示唆された。後期層および最終層は、ConvNeXtと比較して限定的な活性化パターンを示した。

    CLIPモデルでは、視覚野から側頭葉にかけての領域と強く相関することが示された。初期層から中間層にかけての広範な活性化は、CLIPの学習方法の特性を反映している可能性がある。

    #side_by_side(
      grid(
        image("./image/convnext_hierarchical_fwe_p0.05.png", width: 120%),
        image("./image/clip_hierarchical_fwe_p0.05.png", width: 120%),
      ...
      <fig:roi_barplot>,
      "a. DNNモデルの層グループ別脳活動（FWE補正 p < 0.05）。各行は異なる層グループ（初期層+共有、中間層+共有、後期層+共有、最終層+共有）と脳活動の対応を示す。b. DNNモデルの層グループ別ROI効果量。各層（初期・中間・後期・最終）に関連する共有成分を加えた効果量を示す。エラーバーは被験者間の標準誤差（SEM）。★は有意な効果（SVC FWE p < 0.05）。追加の効果量図は @app11[補足] を参照。",
      columns: 1,
      gutter: 2%,
    )
    ...
    本研究の解釈にはいくつかの方法論的限界がある。第一に、CLIPモデルは学習データ（LAION-400M vs ImageNet-1K）と学習タスク（対照学習 vs 分類）の両方がImageNetモデルと異なるため、これらの効果を分離することは困難である。ただし、本研究では画像エンコーダーとして同一のConvNeXt-Baseアーキテクチャを使用しており、画像処理部分の構造的な交絡は排除されている。一方、CLIPではTransformerベースのテキストエンコーダーとの対照学習により、言語的・意味的情報が画像表現に統合されており、これがCLIPの中間層で言語・情動関連領域との対応が見られた理由と考えられる。今後の研究では、対照学習で学習したImageNetモデルなどを用いて、学習データと学習方法の効果を分離して検討する必要がある。
    ...
    本研究は、DNNを用いた計算論的アプローチにより、食品画像からの主観的価値計算における視覚情報処理の階層構造を明らかにした。fMRIデータとDNNとの比較では、初期視覚野（V1）においてノイズ上限値の65-69%を説明できる一方、vmPFCでは4-8%に留まった。この結果は、現在のDNNモデルが視覚処理は捉えられるが、価値計算の神経基盤は十分に捉えられていないことを示している。ConvNeXtは視覚処理階層に沿った対応（初期層-V1、後期層-vmPFC）を示し、CLIPは中間層で情動・言語関連領域（扁桃体、島皮質、IFG、左角回）との対応を示した。CLIPモデルが最も高い予測精度（r = 0.77）と脳活動説明力を示したことは、視覚-言語統合が食品評価において重要な役割を果たすことを示唆している。これらの知見は、食品選択の神経計算基盤の理解に貢献し、将来的には肥満や摂食障害などの食事関連疾患への応用が期待される。

    = 謝辞
    本研究はJSPS科研費JP22K21357（鈴木真介）の助成を受けて実施された。

    研究全般にわたりご指導いただいた鈴木真介氏、副査としてご指導いただいた永山晋氏、研究の構想・設計および刺激提示プログラムの作成にご尽力いただいた菅原通代氏と間野陽子氏、研究の構想と設計にご協力いただいた遠山朝子氏、実験の実施にご協力いただいた松田優羽馬氏と中谷光希氏、参加者募集にご協力いただいた渡辺貴美子氏、貴重なコメントをいただいた山下祐一氏、一部プログラムをご共有いただいた小島大樹氏、ゼミや研究室で貴重なご意見をいただいた南條啓孝氏、大西健太氏、Boxuan Song氏、渡辺千尋氏、北川輝馬氏、Zhe Hu氏に深く感謝申し上げる。

    = コードの公開
    本研究で使用した解析コードは #link("https://github.com/edegp/food-reward-analysis")[GitHub] で公開している。
