...
画像を学習データとテストデータに分け、学習データで8分割クロスバリデーションのグリットサーチを行い最適な正則化パラメータを求めた。正則化パラメータで学習したモデルで層ごとに予測精度を算出し、どの層がどの特徴をよく表現しているかを評価した。予測精度はピアソンの相関係数を使用した。栄養属性（protein_100g、fat_100g、carbs_100g、grams_total）については、一部の画像で値が欠損していたため、これらの属性の予測では欠損値を含む画像を除外して分析を行った。
